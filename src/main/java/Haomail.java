...
public class Haomail extends compose{
    ...
    public static void main(String... args) throws IOException, MessagingException, GeneralSecurityException{
      ...
      /*    
        ...
        //List<String> labelsid = Arrays.asList("INBOX", "SENT");
          // getMimeMessage( service,  user,  "166a98fb0f26467d");
    ...
    public static List<Message> listMessagesWithLabels(Gmail service, String userId,
      ...
      ListMessagesResponse response = service.users().messages().list(userId)
          .setLabelIds(labelIds).setMaxResults(Long.valueOf(7)).execute();
      ...
      //  MessagePartHeader part = message.getPayload().getHeaders();
       // System.out.println(StringUtils.newStringUtf8(Base64.decodeBase64(part.getBody().getData())));
        ...
        for(;fi<n+1;fi++)
        {
           ...
           // System.out.println(StringUtils.newStringUtf8(Base64.decodeBase64(message.getPayload().getParts().get(0).getBody().getData())));
          //  bodi[fi] = (StringUtils.newStringUtf8(Base64.decodeBase64(message.getPayload().getParts().get(0).getBody().getData())));
         ...
         for (MessagePart msgPart : message.getPayload().getParts()) {
           if (msgPart.getMimeType().contains("text/html"))
        	   bodi[fi] = (new String(Base64.decodeBase64(msgPart.getBody().getData())).toString());
         }
        //  frm[fi] = (email.getFrom()[0]);
       // 	subb[fi] = (email.getSubject());
            
           // MimeMessage email  = getMimeMessage(service,  "me",  messageId);

        }
        
    //  System.out.println("Message snippet: " + mess       age.getPayload());
